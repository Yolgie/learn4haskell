...
mid :: Int -> Int -> Int -> Int
mid x y z
    | (x > min y z && x < max y z) || x == y || x == z = x
    | (z > min x y && z < max x y) || z == y = z
    | otherwise = y
...
sumLast2 :: Int -> Int
sumLast2 n =
    let beforeLastDigit = lastDigit (div (abs n) 10)
        in (lastDigit n) + beforeLastDigit
